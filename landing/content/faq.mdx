...
## Where are my custom snippets stored?

By default a [custom snippet](/docs/custom-snippets) is **private** — kept in your
VS Code workspace state, scoped to the repo, and never committed. Choosing **Share
with team** promotes it into `snapds.config.json` (`customSnippets`), which is
committed and shared like the rest of your Snapds config. Sharing writes the
captured source into version control, so it's always an explicit opt-in.
