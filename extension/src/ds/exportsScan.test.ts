...
test('reuses a caller-supplied program instead of building its own', () => {
  // doIntrospect builds one program per package and shares it here to avoid a
  // second ts.createProgram; the result must match the self-built path exactly.
  const program = ts.createProgram([FIXTURE], buildCompilerOptions());
  const names = enumerateComponentExports(FIXTURE, undefined, program)
    .map((e) => e.name)
    .sort();
  assert.deepEqual(names, ['Keyboard', 'Text']);
});
